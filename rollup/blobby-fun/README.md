...
```bash
# For Linux
docker build -t blobby-fun:latest .

# For Mac with Apple Silicon chips
docker buildx build --platform linux/amd64,linux/arm64 -t blobby-fun:latest .
```
...
docker run --env-file .env -v ./db.sqlite:/app/db.sqlite -p <HOST_PORT>:<CONTAINER_PORT> --name=blobby-fun -it blobby-fun:latest

# If using other URI based datastores
docker run --env-file .env -p <HOST_PORT>:<CONTAINER_PORT> --name=blobby-fun -it blobby-fun:latest
```
